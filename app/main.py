...
def check_content_safety(text: str) -> dict:
    ...
    # Extract category severity scores from the result
    if hasattr(result, "categories_analysis") and result.categories_analysis:
        for category in result.categories_analysis:
            # The category object has 'category' (the name) and 'severity' (the score)
            cat_name = category.category
            cat_severity = category.severity
            categories[cat_name] = cat_severity
    
    # Determine safety: True if all categories have severity 0
    is_safe = all(sev == 0 for sev in categories.values()) if categories else True
